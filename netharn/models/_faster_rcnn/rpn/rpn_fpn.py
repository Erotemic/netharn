"""
CommandLine:
    python ~/code/netharn/netharn/models/faster_rcnn/rpn/rpn_fpn.py all
...
from netharn.models.faster_rcnn.rpn.proposal_layer_fpn import _ProposalLayer_FPN
from netharn.models.faster_rcnn.rpn.anchor_target_layer_fpn import _AnchorTargetLayer_FPN
    ...
    def forward(self, rpn_feature_maps, im_info, gt_boxes, num_boxes):
        ...
        for i in range(n_feat_maps):
            ...
            rpn_cls_prob = self.reshape(
                rpn_cls_prob_reshape, self.nc_score_out)
            ...
            rpn_cls_scores.append(rpn_cls_score.permute(
                0, 2, 3, 1).contiguous().view(batch_size, -1, 2))
            rpn_cls_probs.append(rpn_cls_prob.permute(
                0, 2, 3, 1).contiguous().view(batch_size, -1, 2))
            rpn_bbox_preds.append(rpn_bbox_pred.permute(
        ...
        if self.training:
            ...
            _ = rpn_data[1:]
            rpn_bbox_targets, rpn_bbox_inside_weights, rpn_bbox_outside_weights = _
...
if __name__ == '__main__':
    """
    CommandLine:
        python -m netharn.models.faster_rcnn.rpn.rpn_fpn all
    """
    import xdoctest
