...
class MatchingCocoDataset(torch.utils.data.Dataset):
    def __init__(self, sampler, coco_dset, workdir=None, augment=False, dim=416):
        ...
        if augment:
            ...
            self.hack_inf_gen = balanced_infinite_generation(pccs)
    ...
    def __getitem__(self, index):
        import graphid
        if self.augmenter:
            # hack in a richer selection of example pairs
            edge, label = next(self.hack_inf_gen)
            (aid1, aid2) = edge
        else:
...
def balanced_infinite_generation(pccs):
    """
    Note: does not take into account incomparable

    >>> pccs = list(map(frozenset, [{1, 2, 3}, {4, 5}, {6}]))
    >>> gen = (balanced_infinite_generation(pccs))
    >>> next(gen)
    """
    import graphid
    pos_gen = generate_positives(pccs)
    neg_gen = generate_negatives(pccs)
    while True:
        try:
            yield next(pos_gen), graphid.core.POSTV
        except StopIteration:
            pos_gen = generate_positives(pccs)
            yield next(pos_gen), graphid.core.POSTV
        try:
            yield next(neg_gen), graphid.core.NEGTV
        except StopIteration:
            neg_gen = generate_negatives(pccs)
            yield next(neg_gen), graphid.core.NEGTV
...
def generate_positives(pccs):
    generators = {i: nh.util.random_combinations(pcc, 2)
...
def generate_negatives(pccs):
    ...
    while unfinished:
        ...
        if unfinished is True:
            combos = nh.util.random_combinations(pccs, 2)
        else:
            combos = unfinished
        ...
        for pcc1, pcc2 in combos:
            ...
            if key not in generators:
                generators[key] = nh.util.random_product([pcc1, pcc2])
            gen = generators[key]
            try:
                edge = e_(next(gen))
                yield edge
